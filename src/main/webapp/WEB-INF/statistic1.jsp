		...
		<script>
			...
			<%-------------------------------------------*/--%>
					Morris.Bar({
						...
						data: [
							<c:forEach items="${balancelist}" var="row" varStatus="status">
                    {
                        y: '${row.type}',
                        a: ${row.goodnumber},
                        b: ${row.midnumber},
                    ...
                    <c:if test="${!status.last}" >,</c:if>
						...
						xkey: 'y',
						ykeys: ['a','b','c','d','e','f'],
						labels: ['顺丰快递的好评', '中评','差评','京东快递的好评','中评','差评'],
						hideHover: 'auto',
						resize: true
					});
					...
					Morris.Donut({
						...
						data: [
							<c:forEach items="${specialtxt}" var="row1" varStatus="status">
							{
								label:"${row1.get("content")}",
								value:${row1.get("num")}
